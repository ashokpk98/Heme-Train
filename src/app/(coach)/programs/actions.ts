...
export async function updateSet(
  ...
  await db.update(prescribedSets).set(patch).where(eq(prescribedSets.id, setId));
  refresh(programId);
}
...
export async function addSet(programId: string, slotId: string) {
  ...
  const { id: _id, ...clone } = last ?? {
    ...
    rpeTarget: null,
    rirTarget: null,
    velocityTargetMin: null,
    velocityTargetMax: null,
    velocityLossThresholdPct: null,
    tempo: null,
    restSeconds: null,
    durationSec: null,
    distanceM: null,
...
export async function deleteSet(programId: string, setId: string) {
  ...
  await db.delete(prescribedSets).where(eq(prescribedSets.id, setId));
  await renumberSets(row.slotId);
  refresh(programId);
}

async function renumberSets(slotId: string) {
...
export async function addSlot(
  ...
  await db.insert(prescribedSets).values({
    ...
    distanceM: !tracks("reps") && !tracks("time") && tracks("distance") ? 20 : null,
...
export async function deleteSlot(programId: string, slotId: string) {
  ...
  await db.delete(exerciseSlots).where(eq(exerciseSlots.id, slotId));
  await relabelSlots(row.blockId);
  refresh(programId);
}
...
export async function reorderSlots(
  ...
  await relabelSlots(sessionBlockId);
  refresh(programId);
}

/** Keeps A1/A2/B1 labels in step with the block and slot order. */
async function relabelSlots(sessionBlockId: string) {
  const [block] = await db
...
export async function updateSessionBlock(
  ...
  await db.update(sessionBlocks).set(patch).where(eq(sessionBlocks.id, blockId));
  refresh(programId);
}
          ...
          await db.insert(prescribedSets).values(
            srcSlot.sets.map((s) => {
              const progressed = rule
                ? (progressSet(s as PrescriptionInput, rule, weekOffset) as PrescriptionInput)
...
export async function duplicateSession(
  ...
  const src = await db.query.sessions.findFirst({
  ...
  for (const b of src.blocks) {
    ...
    for (const sl of b.slots) {
      ...
      if (sl.sets.length) {
        await db.insert(prescribedSets).values(
          sl.sets.map(({ id: _id, slotId: _slotId, ...rest }) => ({
            ...rest,
            slotId: ns.id,
          })),
        );
